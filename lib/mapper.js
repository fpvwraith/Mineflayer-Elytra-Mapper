...
function createSpawnMapper(deps) {
    ...
    let recoverKills = 0;          // consecutive recovery /kills that haven't gotten us back to a base yet (bounded by MAX_RECOVER_KILLS)
    ...
    // Nearest not-yet-covered cell to where the bot actually is right now.
    // Deliberately NOT "next in a list sorted by distance from 0,0": the base
    // sits OUTSIDE the 5k zone, so a spawn-ordered list would always aim at
    // (0,0) ~5k away, never arrive within one elytra, never advance, and re-fly
    // the same line forever. Nearest-first means every flight makes progress and
    // coverage grows outward from wherever the bot starts.
    function pickTarget() {
        const pos = botPos();
        if (!pos) return null;
        // Heading: current velocity while flying, else aim inward toward 0,0.
        const bot = getBot();
        ...
        // Prefer a long STRAIGHT sweep: among uncovered cells that lie ahead of
        // the current heading and within ~one lane of it, take the FARTHEST one,
        // so the bot commits to a long lawnmower pass instead of zig-zagging to
        // whatever's nearest (which burned most of its airspeed on turns - net
        // ~8 b/s vs ~33 b/s cruise). When nothing lies ahead (end of a pass),
        // fall back to the nearest uncovered cell to start the next lane.
        ...
        for (const w of waypoints) {
            ...
            const proj = dx * hx + dz * hz;            // distance ahead along heading
            const perp = Math.abs(dx * -hz + dz * hx); // sideways offset from the heading line
            if (proj > 30 && perp <= GRID_STEP && proj > sweepProj) { sweepProj = proj; sweep = w; }
        }
        if (!nearest) { covered.clear(); return null; } // whole zone done - fresh pass next tick
        // Highways are a single on-axis lane, so "nearest uncovered" already IS a
        // straight pass (no zig-zag to worry about) - and picking nearest makes the
        // bot go to the corridor's NEAR end first (arcing around spawn via the
        // keep-out) then sweep outward, instead of beelining to the farthest cell
        // and cutting diagonally across the spawn zone (the red-vs-green path). The
        // farthest-ahead sweep stays for the spawn GRID, where it avoids lane zig-zag.
        if (mappingMode !== 'spawn' && !mappingMode.startsWith('ring')) return nearest; // highways: single-lane, nearest IS the straight pass
        return sweep || nearest; // spawn/ring area fill: prefer a long lawnmower pass
    }
    ...
    function steerFlight(target) {
        ...
        try {
            ...
            const steerPt = mappingMode === 'spawn' ? target : routeAroundKeepout(pos, target);
    ...
    async function tick() {
        ...
        switch (state) {
            ...
            case 'acquire': {
                if (prepStep === 'start') {
                    ...
                    acquireElytra().then(ok => {
                        if (!active) return;
                        prepStep = null;
                        if (ok) { recoverKills = 0; setState('gotolaunch'); teleportSentAt = 0; return; }
            ...
            case 'recover': {
                ...
                if (recoverKills >= MAX_RECOVER_KILLS) {
                    ...
                    const why = d <= HOME_BLOCKED_RADIUS
                        ? `the far bed looks genuinely gone - deaths keep landing me at world spawn (${Math.round(pos.x)},${Math.round(pos.z)}). Re-sleep the bed at the far base`
                    ...
                    sendLog(`🛑 [mapper] auto-stopped after ${recoverKills} recovery /kills: ${why}, then \`!map on ${mappingMode}\`.`);
                    stop();
                    return;
                }
                recoverKills++;
            ...
            case 'launch': {
                ...
                // Keep walking toward the current target (so we leave the platform
                // heading roughly the right way).
                try {
                    const t = currentTargetPt || pickTarget();
                    if (t) {
            ...
            case 'mapping': {
                ...
                if (!currentTargetPt) currentTargetPt = pickTarget();
    ...
    function start(mode, opts) {
        ...
        if (mode === 'rotate') {
            ...
            if (!mode.startsWith('ring') && !(mode in MAPPING_MODES)) return `unknown mode "${mode}" (valid: ${Object.keys(MAPPING_MODES).join(', ')}, ring-nw|ne|se|sw, rotate)`;
            rotating = false;
            mappingMode = mode;
        }
